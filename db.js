...
try { db.exec(`ALTER TABLE comments ADD COLUMN parent_id TEXT DEFAULT NULL`); } catch(_){}
...
function addComment(userId, postId, content, parentId) {
  const id = uid();
  db.prepare(`INSERT INTO comments(id,user_id,post_id,content,parent_id) VALUES(?,?,?,?,?)`).run(id, userId, postId, content, parentId||null);
  const post = db.prepare(`SELECT user_id FROM posts WHERE id=?`).get(postId);
  if (parentId) {
    // Notify parent comment author
    const parent = db.prepare(`SELECT user_id FROM comments WHERE id=?`).get(parentId);
    if (parent && parent.user_id !== userId) addNotification(parent.user_id, userId, 'comment', postId, content.slice(0,80));
  } else if (post && post.user_id !== userId) {
    addNotification(post.user_id, userId, 'comment', postId, content.slice(0,80));
  }
...
function getPostComments(postId, viewerId) {
  const allCmts = db.prepare(`SELECT c.*, u.username, u.display_name, u.avatar_url, u.belt,
  ...
  topLevel.forEach(c => { c.replies = allCmts.filter(r => r.parent_id === c.id); });
  return topLevel;
}
