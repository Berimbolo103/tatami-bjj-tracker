...
function optionalAuth(req, res, next) {
  const token = (req.headers.authorization || '').replace('Bearer ', '');
  if (token) req.user = db.getUserByToken(token); next();
}
function requireAdmin(req, res, next) {
  if (req.headers['x-admin-token'] !== ADMIN_TOKEN) return res.status(401).json({ error: 'Unauthorized' }); next();
}

app.get('/api/health', (req, res) => res.json({ status: 'ok', env: ENV, timestamp: new Date().toISOString() }));
...
app.post('/api/auth/register', (req, res) => {
  ...
  const r = db.registerUser({ username, email, password, display_name });
...
app.post('/api/auth/login', (req, res) => {
  ...
  const r = db.loginUser({ login, password });
...
app.get('/api/profile/:userId', optionalAuth, (req, res) => {
  ...
  if (req.user) { profile.friendship = db.getFriendshipStatus(req.user.id, req.params.userId); profile.is_self = req.user.id === req.params.userId; }
  res.json(profile);
});
...
app.post('/api/profile/avatar', auth, upload.single('avatar'), (req, res) => {
  ...
  const url = `/uploads/${req.file.filename}`; db.updateProfile(req.user.id, { avatar_url: url }); res.json({ avatar_url: url });
});
...
// ── UNIFIED POSTS (sessions + posts = one) ──────────────────────────────────
...
// Sessions view (filtered posts where is_session=1)
...
app.get('/api/posts/:id/comments', auth, (req, res) => res.json(db.getPostComments(req.params.id)));
...
app.post('/api/techniques/submit', auth, (req, res) => {
  ...
  const r = db.submitTechnique({ name, category, description, submitted_by: req.user.username });
  if (r.alreadyExists) return res.status(409).json({ error: `"${name}" already exists`, status: r.status });
  res.json({ id: r.id, status: 'pending' });
});
app.get('/api/techniques/my-submissions', auth, (req, res) => res.json(db.getSubmissionsByUser(req.user.username)));
...
app.listen(PORT, () => {
  console.log(`\n  🥋 TATAMI v3 — ${ENV} — http://localhost:${PORT}\n`);
});
module.exports = app;
